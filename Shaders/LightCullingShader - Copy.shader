...
void main() {
	...
	depth = LinearizeDepth(depth);//projection[3][2] / (projection[2][2] + 2.0 * depth - 1.0);//(0.5 * projection[3][2]) / (depth + 0.5 * projection[2][2] - 0.5);
	...
	for (uint i = 0; i < passCount; i++) {
		...
		float radius = lightBuffer.data[lightIndex].size;
	...
	if (gl_LocalInvocationIndex == 0) {
		...
		for (uint i = 0; i < visibleLightCount; i++) {
			visibleLightIndicesBuffer.data[offset + i + 1].index = visibleLightIndices[i];
		}
