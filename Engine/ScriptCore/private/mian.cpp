...
std::string someCppNativeFunction(std::string* s, int i) {
	std::ostringstream repeated;
	std::fill_n(std::ostream_iterator<std::string>(repeated), i, *s);
	*s = "Hello, repeated";
	return repeated.str();
}

std::string someCpp(std::string s, int i) {
	std::ostringstream repeated;
	std::fill_n(std::ostream_iterator<std::string>(repeated), i, s);
	return repeated.str();
}
