...
CloudParticle::CloudParticle() : ParticleSystem()
{
	...
	SetConstructor([](Particle& p) {
		...
		p.velocity = Vector::RandomUnitVectorInCone({ 0, 0, 1 }, 90.f, { 0, 1, 0 }) * 0.3;
		p.rotationRate = Vector(RandomFloatInRange(-2, 2), 0.f, 0.f);
		p.rotation = RandomFloatInRange(0, 360);
		p.max_lifetime = 25.f;
		p.location = { RandomFloatInRange(-35, 35), RandomFloatInRange(-35, 35), RandomFloatInRange(-15, 15) };
		p.alpha = 1.f;
		p.initialColour = Vector(1.1);
		});
	...
	SetSpawner(ParticleSystemConstruction::MakeSpawnRate(this, 2.3f));
}
